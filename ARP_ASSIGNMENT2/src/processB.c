...
{   
    int shared_mem_descriptor  = shm_open(SHARED_MEM_NAME, O_RDONLY, SHM_MODE); // Open shared memory allocation
    ...
    sem_t *write_semaphore  = sem_open(WRITE_SEM_NAME, 0);  // Open semaphores
    sem_t *read_semaphore  = sem_open(READ_SEM_NAME, 0);    // Open semaphores
