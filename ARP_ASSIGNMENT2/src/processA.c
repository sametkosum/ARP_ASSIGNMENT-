...
{   
    int shared_mem_descriptor  = shm_open(SHARED_MEM_NAME, O_CREAT | O_RDWR, SHM_MODE);   // Allocating shared memory

    if (shared_mem_descriptor  == -1)
    {
        perror("Unable to open the shared memory allocation\n");
        return 1;
    }
    ftruncate(shared_mem_descriptor , sizeof(struct msg));     // Setting size of shared memory allocation
    ...
    while (TRUE)
    {
        ...
        else if(cmd == KEY_LEFT || cmd == KEY_RIGHT || cmd == KEY_UP || cmd == KEY_DOWN)
        {
            ...
            draw_circle();
            REMOVE_DRAWbmp(bmp, previous_x , previous_y);   // Clear previous circle
            for (int i = 0; i < WIDTH; i++)                 // Also reset shared memory data
            {
                for (int j = 0; j < HEIGHT; j++)
            ...
            for (int i = 0; i < WIDTH; i++)                  //Store the circular information within thehared memory
            {
                for (int j = 0; j < HEIGHT; j++)
                {
                    rgb_pixel_t *pixel = bmp_get_pixel(bmp, i, j);
